...
export default function AddressProperty() {
  ...
  const filteredCities = cities.filter((city) => String(city.province_id) === selectedProvinceId);

  useEffect(() => {
    if (selectedCityId && !filteredCities.find((c) => String(c.id) === selectedCityId)) {
      setValue("city", "");
    }
    ...
    <div className="mb-12">
      ...
      <div className="grid grid-cols-2 gap-4">
        <FormControl fullWidth size="small" error={!!errors.province}>
          ...
          <FormHelperText>{String(getErrorMessage(errors.province) ?? "")}</FormHelperText>
        </FormControl>

        <FormControl fullWidth error={!!errors.city} disabled={!selectedProvinceId} size="small">
          ...
          />
          <FormHelperText>{String(getErrorMessage(errors.city) ?? "")}</FormHelperText>
        </FormControl>
