...
export default function UserInformation({ userId }: UserIdProps) {
  const {
    ...
    formState: { errors },
    reset,
  } = useFormContext<ProfileModel>();

  const phone = useNumericField("phoneNumber", 11, "09");

  const updateUser = async (data: ProfileModel) => {
    try {
  ...
  const onSubmit = (data: ProfileModel) => {
    const filtered: ProfileModel = {};
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === "address") {
        ...
        filtered[key as Exclude<keyof ProfileModel, "address">] = value;
      }
    });
